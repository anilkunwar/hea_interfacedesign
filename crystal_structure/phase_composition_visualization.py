...
def load_data(uploaded_file=None, local_path=None, github_url=None):
    try:
        ...
        if df[['xAl', 'xNi', 'xCr', 'xCo', 'xFe']].isnull().any().any():
            ...
            logger.error("CSV contains non-numeric or NaN values")
            return None
...
def get_color_values(df):
    ...
    for _, row in df.iterrows():
        ...
        try:
            ...
            logger.error(f"Invalid alloy name format: {alloy_name}")
            return None
        if structure == 'FCC':
...
def main():
    ...
    # Add scatter points with phase in hover
    fig.add_trace(
        go.Scatterternary(
            a=df['p_Al_norm'],
            b=df['p_CoCr_norm'],
            ...
            hovertemplate="Alloy: %{text}<br>Phase: %{customdata}<br>Al: %{a:.3f}<br>Co+Cr: %{b:.3f}<br>Fe+Ni: %{c:.3f}"
        )
    )
    ...
    fig.update_layout(
        title=dict(text="Ternary Diagram of AlyCoCrFeNi Alloy", font=dict(size=font_size + 2)),
        showlegend=False,
